...
typedef void (*cr_system_on_reader_receive_hw_serial_number_cb_t)(void *impl, struct cr_system_t *system, char const *sn);
...
typedef void (*cr_system_on_reader_receive_fw_version_cb_t)(void *impl, struct cr_system_t *system, char const *version);

// Callback invoked when the reader provides the charge cycle count.
typedef void (*cr_system_on_reader_receive_charge_cycle_count_cb_t)(void *impl, struct cr_system_t *system, uint32_t charge_count);

// Callback invoked when the reader provides the capabilities
typedef void (*cr_system_on_reader_receive_capabilities_cb_t)(void *impl, struct cr_system_t *system, uint8_t const *capabilities,
                                                              size_t capabilities_len, bool capabilities_supported);
...
typedef void (*cr_system_on_reader_receive_k400_has_reset_cb_t)(void *impl, struct cr_system_t *system, uint32_t number_of_resets);

// Callback invoked when SPE sends RIT_system_spe_disconnect_ack
typedef void (*cr_system_on_reader_receive_spe_disconnected_cb_t)(void *impl, struct cr_system_t *system);

typedef void (*cr_system_on_reader_receive_reader_error_cb_t)(void *impl, struct cr_system_t *system, crs_system_reader_error_t reader_error);
...
typedef struct cr_system_event_api_t
{
    ...
    // When SPE processed RIT_system_spe_disconnect it sends back RIT_system_spe_disconnect_ack
    cr_system_on_reader_receive_spe_disconnected_cb_t on_reader_receive_spe_disconnected;
...
cr_system_result_t cr_system_set_hardware_platform_feature(struct cr_system_t *system,
...
cr_system_result_t cr_system_send_disconnect_notification(struct cr_system_t *system);

// Set one reader feature flag value (but don't yet send to reader)
cr_system_result_t cr_system_set_reader_feature_flag(struct cr_system_t *system, const char *feature_name, uint8_t value);
